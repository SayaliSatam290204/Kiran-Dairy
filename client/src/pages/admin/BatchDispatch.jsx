    ...
    try {
      ...
      if (customizePerShop) {
        ...
        for (const shopId of selectedShops) {
          ...
          await adminApi.createBatchDispatch(dispatchData);
        }
        toast.success(
          `Dispatches created successfully for ${selectedShops.length} shop(s)!`
        );
      } else {
        // Create single batch dispatch for all shops with same items
        const dispatchData = {
          ...
          isBatchDispatch: selectedShops.length > 1
        };

        await adminApi.createBatchDispatch(dispatchData);
        toast.success(
          `Dispatch created successfully for ${selectedShops.length} shop(s)!`
        );
      }
